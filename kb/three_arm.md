...
### 19.8 The learning ties (2026-08-17)

The 2026-08-16 champion at the flat-prior corner (m_b = m_c = 0,
tau_bb = tau_cc = 1e-3, tau_bc = 0), where symmetry forces all three learning
numbers equal:

    l_ab 287    l_ac 157    l_bc 695    policy (0.00, 0.50, 0.50)

The wedge fold makes the VALUE exactly S3-invariant, but the derivatives read
at a wall are one-sided and nothing graded them: the value ties hold first
mean-derivatives only, concavity holds a sign, and the subsolution residual
grades max H's VALUE, which by the envelope theorem is second-order
insensitive to the argmax exactly where the argmax is most sensitive to the
l-ratios. Worse, the one-sided constraint pays for inflating an l (a bigger
max H is slack), which fits the direction (one l pumped 3x) and the timing:
the arena assert on the flat-prior opening stopped holding at the 2026-08-16
promotion. Unrepresentable at N = 2, where the symmetric-point parabola is
`-l x^2 + l x` and any single l cancels out of the argmax.

The fix is the value ties' second-order siblings, same RidgeSample draws,
same mirror pairing, one derivative order up. Each wall's swap acts on
(l_ab, l_ac, l_bc) as a transposition plus a fixed element, three equalities
per wall against the mirror state; BOTH walls are needed, because the corner's
symmetry is generated by the two transpositions -- the treatment tie alone
leaves l_bc free and the corner opens (0, 1/2, 1/2), which is exactly what the
probe found. Scored in the RELATIVE form `((l - l') / (|l| + |l'| + eps))^2`:
an equality constraint, so the positive per-point rescale is the sanctioned
exception to never-scale-the-residual (it cannot move the zero set), and
without it the floor decade's `l ~ 1/det` owns the gradient. Exactly zero on
the dead solution, so the degeneracy floor is untouched.

Champion medians over 7 draws at wiring: control 9.6e-2, treatment 4.5e-2
(against value ties at ~5e-6 and 2e-6 -- the walls held to first order and
were wide open at second). `LEARNING_TIE_WEIGHT = 0.1` landed the pair at the
climb term's scale; it equilibrated short of target (treatment flat at ~7e-3
for 7k iterations, opening stuck) and the 10x raise to 1.0 finished the job.

RETRAIN AND PROMOTION (2026-08-18). Off the 2026-08-16 champion: weight
0.1 -> 1.0, SLACK_PRICE 0.02 for the reshaping leg then 0 for the polish,
lr 1e-4 then 3e-5. Probe states: the tau_bc = 0 "corner" is NOT exchangeable
(zero direct b-c information; control-light is legitimate there). The state
that must open 1/3 is the ARM-SYMMETRIC prior (tau_bb = tau_cc = p,
tau_bc = -p/2), which is what the arena's Pinn entrant builds at the flat
prior. Result, medians over 5 draws at 8192, old -> new:

    opening (arm-symmetric, p = 1e-3)  (0.446, 0.376, 0.178) -> (0.350, 0.345, 0.306)
    learning ties ctl / trt            9.5e-2 / 4.2e-2 -> 4.9e-5 / 2.1e-3
    violation 2.2e-5 -> 9.6e-5    sup 1.7e-2 -> 4.9e-2    premium 0.4704 -> 0.4664

ARENA: 20,000 paired reps at production parameters, frozen baselines
bit-identical across sweeps: new - old = -13 +/- 341, statistically zero;
both ~4,400 ahead of Thompson (17.9k vs 22.3k). The bound loosened 4x for
the symmetric opening and the arena is indifferent -- the same pattern as
two_arm_drift's subsolution swap. Promoted on the policy fix with the arena
