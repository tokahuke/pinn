...
"""
*Not* data/ itself: the champions live there under the same names a pod would write,
and a fetch that clobbers two_arm.pt is unrecoverable.
"""
...
def fetch(pod: Pod) -> str:
    ...
    # Not fatal: a pod whose ssh has died still has to be destroyable, or it bills
    # forever.
...
def down(name: str, yes: bool, no_fetch: bool) -> None:
    ...
    Destroying is the only thing that stops the billing: a stopped pod still costs.
    """
    ...
    # Stopped *before* the pod goes, so it is not left retrying against a corpse: it
    # reconnects with backoff and would never give up on its own.
    stopped = Daemon(name).stop()
    click.echo(
